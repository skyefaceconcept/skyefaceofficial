                    ...
                    <li class="nav-item dropdown">
                        <a id="userDropdown" class="nav-link d-flex align-items-center" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" aria-label="User menu">
                            @if(auth()->user()->profile_photo_path && \Illuminate\Support\Facades\Storage::disk('public')->exists(auth()->user()->profile_photo_path))
                                <img src="{{ asset('storage/' . auth()->user()->profile_photo_path) }}" alt="Profile" class="rounded-circle" style="width: 32px; height: 32px; object-fit: cover;">
                            @else
                                <i class="fa fa-user-circle" style="font-size: 24px;"></i>
                            @endif
                            <span class="d-none d-md-inline ml-2">{{ auth()->user()->fname ?? auth()->user()->name }}</span>
                        </a>
