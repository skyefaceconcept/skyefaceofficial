...
<head>
	...
	<meta name="description" content="Contact Us — {{ config('company.name') }}">
	<meta name="author" content="{{ config('company.name') }}">

	<title>{{ config('company.name') }} — Contact Us</title>
...
<body id="page-top">
				...
				<div class="col-lg-8 mx-auto">
					...
					@if (session('status'))
						<div class="alert alert-success">
							...
							<div class="mt-2">{{ session('status') }}</div>
						</div>
					@endif
							...
							<div class="contact-form">
								...
								<form action="{{ route('contact.store') }}" method="POST" novalidate>
									...
									<div class="form-group">
										...
										<input type="tel" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone') }}" placeholder="+1 (555) 000-0000">
										@error('phone')
											<span class="invalid-feedback">{{ $message }}</span>
										@enderror
									</div>
									...
									<div class="form-group">
										...
										<textarea class="form-control @error('message') is-invalid @enderror" id="message" name="message" placeholder="Tell us more about your inquiry..." required>{{ old('message') }}</textarea>
										@error('message')
											<span class="invalid-feedback">{{ $message }}</span>
										@enderror
									</div>
						...
						<div class="col-lg-4">
							<div class="contact-info">
								...
								<div class="contact-info-item">
									...
									<div>
										<h5>Address</h5>
										<p>{{ config('company.address', 'Your Company Address') }}</p>
									</div>
								</div>
								...
								<div class="contact-info-item">
									...
									<div>
										<h5>Phone</h5>
										<p><a href="tel:{{ str_replace([' ', '(', ')', '-'], '', config('company.phone', '')) }}" style="color: #7f8c8d; text-decoration: none;">{{ config('company.phone', 'Your Phone Number') }}</a></p>
									</div>
								</div>
								...
								<div class="contact-info-item">
									...
									<div>
										<h5>Email</h5>
										<p><a href="mailto:{{ config('company.email', '') }}" style="color: #7f8c8d; text-decoration: none;">{{ config('company.email', 'your@email.com') }}</a></p>
									</div>
								</div>
								...
								<div class="contact-info-item">
									...
									<div>
										...
										<p>
											Monday - Friday<br>
											<strong>{{ config('company.working_hours.weekday_start', '9:00 AM') }} - {{ config('company.working_hours.weekday_end', '6:00 PM') }}</strong>
										</p>
										<p style="margin-top: 10px;">
											Saturday<br>
											<strong>{{ config('company.working_hours.saturday_start', '10:00 AM') }} - {{ config('company.working_hours.saturday_end', '4:00 PM') }}</strong>
										</p>
			...
			<div class="row mt-5">
				<div class="col-md-4 text-center">
					<div style="padding: 30px; background: #f8f9fa; border-radius: 8px;">
						...
						<p style="color: #7f8c8d;">{{ config('company.address', 'Your Company Address') }}</p>
					</div>
				</div>
				...
				<div class="col-md-4 text-center">
					<div style="padding: 30px; background: #f8f9fa; border-radius: 8px;">
						...
						<p style="color: #7f8c8d;"><a href="tel:{{ str_replace([' ', '(', ')', '-'], '', config('company.phone', '')) }}" style="color: #7f8c8d; text-decoration: none;">{{ config('company.phone', 'Your Phone Number') }}</a></p>
					</div>
				</div>
				...
				<div class="col-md-4 text-center">
					<div style="padding: 30px; background: #f8f9fa; border-radius: 8px;">
						...
						<p style="color: #7f8c8d;"><a href="mailto:{{ config('company.email', '') }}" style="color: #7f8c8d; text-decoration: none;">{{ config('company.email', 'your@email.com') }}</a></p>
					</div>
				</div>
			</div>
		</div>
	...
	<!-- Script Files -->
	<script src="{{ asset('buzbox/js/jquery.min.js') }}"></script>
	<script src="{{ asset('buzbox/js/bootstrap.bundle.min.js') }}"></script>
	<script src="{{ asset('buzbox/js/jquery.easing.min.js') }}"></script>
	<script src="{{ asset('buzbox/js/scrollreveal.min.js') }}"></script>
	<script src="{{ asset('buzbox/js/script.js') }}"></script>
