...
<div class="container-fluid py-4">
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-4 text-right">
            <a href="{{ route('admin.analytics.page-impressions.export', ['days' => request('days', 30)]) }}" 
            ...
            <div class="card border-left-primary shadow h-100 py-2">
                <div class="card-body">
                    ...
                    <div class="h3 mb-0 font-weight-bold text-gray-800">
                        {{ number_format($totalImpressions ?? 0) }}
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <table class="table table-hover table-striped">
                                ...
                                <tbody>
                                    @foreach($topPages as $page)
                                        <tr>
                                            <td class="pl-3">
                                                <code class="text-primary">{{ $page->page_url ?? $page->page_route ?? 'unknown' }}</code>
                                            </td>
                                            <td>
                                                <small class="text-muted">{{ Str::limit($page->page_title ?? 'Untitled', 50) }}</small>
                                            </td>
                                            <td class="text-right">
                                                <span class="badge badge-primary">{{ number_format($page->impressions ?? $page->total_impressions ?? 0) }}</span>
                                            </td>
                                            <td class="text-right">
                                            ...
                                            <td class="text-center">
                                                   <a href="{{ route('admin.analytics.page-impressions.show', ['page' => $page->page_url ?? $page->page_route, 'start_date' => isset($startDate) ? $startDate->format('Y-m-d') : now()->subDays(30)->format('Y-m-d'), 'end_date' => isset($endDate) ? $endDate->format('Y-m-d') : now()->format('Y-m-d')]) }}"
                                                   class="btn btn-sm btn-info">
                                    ...
                                    <tr>
                                        <td>
                                            <a href="{{ $referrer->referer ?? '#' }}" target="_blank" class="text-primary">
                                                {{ Str::limit($referrer->referer ?? 'Direct', 60) }}
                                                <i class="fas fa-external-link-alt ml-1" style="font-size:0.8em;"></i>
                                            </a>
                                        </td>
                                        <td class="text-right">
                                            <span class="badge badge-info">{{ number_format($referrer->count ?? 0) }}</span>
                                        </td>
                                        <td class="text-right">
                                            {{ number_format(round((($referrer->count ?? 0) / max(1, $totalImpressions ?? 1)) * 100), 2) }}%
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
<script>
    ...
    @if(isset($impressionsByDate) && $impressionsByDate->count() > 0)
    ...
    @if(isset($hourlyData) && $hourlyData->count() > 0)
