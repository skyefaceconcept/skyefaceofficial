        ...
        <form action="{{ route('admin.permissions.update', $permission->id) }}" method="POST">
          ...
          <div class="form-group">
            ...
            <textarea
              ...
              name="description"
              rows="4"
              placeholder="Brief description of what this permission does...">{{ old('description', $permission->description) }}</textarea>
            @error('description')
              <span class="invalid-feedback">{{ $message }}</span>
            @enderror
          </div>
          ...
          <div class="form-group">
            ...
            <select id="route_select" class="form-control @error('route') is-invalid @enderror" required>
              ...
              @foreach($routes as $r)
                <option value="{{ $r['name'] }}" data-uri="{{ $r['uri'] }}" {{ old('route', $permission->route) == $r['name'] ? 'selected' : '' }}>{{ $r['name'] }} &mdash; {{ $r['uri'] }}</option>
              @endforeach
            </select>
            <input type="hidden" id="route" name="route" value="{{ old('route', $permission->route) }}">
            @error('route')
              <span class="invalid-feedback">{{ $message }}</span>
            @enderror
          ...
          <div class="form-group mt-4">
            ...
            <form action="{{ route('admin.permissions.destroy', $permission->id) }}" method="POST" class="d-inline float-right ms-2">
              @csrf
              @method('DELETE')
  ...
  <div class="col-md-4">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <p><strong>Last Updated:</strong> {{ $permission->updated_at->format('M d, Y H:i') }}</p>
        <hr>
